...
export function realWorldValue({id,label,value=null,unit='',record={},asOf,ttlMs=null,context='',missingReason=null}) {
  ...
  return {id,subjectId:record.subjectId??null,label,value:valid?value:null,unit,measurementType,observedAt,receivedAt,calculatedAt,
    sourceId,sourceName,latency:{providerToIngestionMs:observed!==null&&time(receivedAt)!==null&&time(receivedAt)>=observed?time(receivedAt)-observed:null,ingestionToProjectionMs:time(receivedAt)!==null&&now>=time(receivedAt)?now-time(receivedAt):null,ageAtProjectionMs:age},sourceLocation:record.sourceLocation??(point(record.coordinate)?{name:record.stationName??record.locationName??null,coordinate:point(record.coordinate)}:null),
