...
export function weatherMetrics(records,asOf,{location=null,subjectId=null,sourceState=null}={}) {
  const now=timestamp(asOf);
  const samples=rows(records).filter(r=>!r.modelRun&&!r.model&&r.measurementType!=='MODELED'&&r.weatherRole!=='FORECAST_CONTEXT_NOT_FIRE_EVIDENCE'&&timestamp(r.observedAt??r.validAt)!==null&&timestamp(r.observedAt??r.validAt)<=now).map(r=>({...r,observedAt:r.observedAt??r.validAt,subjectId,
    distanceKm:distanceKm(location,r.coordinate)??r.distanceKm,
  ...
  for(const s of samples){const key=(s.source??'')+':'+(s.stationId??s.coordinate?.join(',')??'unknown');if(!byStation.has(key)||timestamp(s.observedAt)>timestamp(byStation.get(key).observedAt))byStation.set(key,s);}
  const usable=s=>['temperatureC','windSpeedKph','humidityPercent'].some(k=>number(s[k])!==null);
  const current=s=>usable(s)&&now-timestamp(s.observedAt)<=WEATHER_TTL&&!/stale|failed|unavailable/i.test(s.sourceState??'');
  const station=[...byStation.values()].sort((a,b)=>Number(current(b))-Number(current(a))||Number(usable(b))-Number(usable(a))||(a.distanceKm??Infinity)-(b.distanceKm??Infinity)||timestamp(b.observedAt)-timestamp(a.observedAt))[0];
  ...
  const metrics=definitions.map(([id,label,key,unit])=>{
    ...
    const same=samples.filter(s=>Boolean(station?.stationId)&&s.stationId===station.stationId&&s.source===station.source);
...
export function thermalMetrics(records,location,asOf,independentFamilies=null,coverage=null) {
  ...
  const metrics=[metric('thermalTime','Latest thermal detection',last?.observedAt??null,'',last,asOf,THERMAL_TTL,context),metric('thermalDistance','Nearest thermal detection',near?.distance??null,'km',near,asOf,THERMAL_TTL,context),metric('frp','Fire radiative power',number(last?.frpMw),'MW',last,asOf,THERMAL_TTL,context),metric('thermalProduct','Satellite / product',last?.product??null,'',last,asOf,THERMAL_TTL,context)];
  ...
  return {metrics,detections,latestThermalDetection:last??null,nearestDetection:near??null};
}
