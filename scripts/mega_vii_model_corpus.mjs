...
const gate=JSON.parse(await readFile('docs/handoffs/mega-vii/model-runtime.json','utf8'));
if(!gate.attempts.some(a=>a.model===model&&a.passed&&a.attempts.length===50))throw new Error('twenty_successful_sentinels_required');
...
const rows=[],out='docs/handoffs/mega-vii/model-corpus.json',identity=x=>JSON.stringify([x.subject,x.field,x.value]),pct=f=>100*rows.filter(f).length/rows.length;
function report(){const times=rows.map(r=>r.latencyMs).sort((a,b)=>a-b),abstain=rows.filter(r=>r.abstentionExpected),intent=rows.filter(r=>r.category==='tool-intent');return{lane:'ACTUAL_LOCAL_MODEL_ON_CONTROLLED_CORPUS_NO_OPERATIONAL_WRITES',model,responseDeadlineMs:5000,casesPlanned:cases.length,casesRun:rows.length,schemaValidPercent:pct(r=>r.schemaValid),factCorrectPercent:pct(r=>r.correct),sourceSpanPercent:pct(r=>r.sourceSupported),abstentionPercent:abstain.length?100*abstain.filter(r=>r.correct).length/abstain.length:null,toolIntentPercent:intent.length?100*intent.filter(r=>r.correct).length/intent.length:null,p50Ms:times[Math.floor(times.length*.5)]??null,p95Ms:times[Math.ceil(times.length*.95)-1]??null,requestErrors:rows.filter(r=>r.error).length,rows};}
