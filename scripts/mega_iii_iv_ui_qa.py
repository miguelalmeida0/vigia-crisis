...
with sync_playwright() as p:
 ...
 for width,height in [(1672,941),(390,844)]:
  ...
  page.locator('.vigia-dialog .icon-button[data-dialog-close]').click();page.get_by_role('button',name='Ask Vigia',exact=True).click();page.get_by_label('Question',exact=True).fill('Which hospital is closest?');page.locator('[data-action="situation-ask"]').click();wait_dom('()=>document.querySelector("[data-situation-output]")?.textContent.includes("Hospital")');shot('hospital-answer-'+str(width))
 ...
 report['browser']=browser.version;(OUT/'ui-qa.json').write_text(json.dumps(report,indent=2));browser.close()
server.shutdown()
