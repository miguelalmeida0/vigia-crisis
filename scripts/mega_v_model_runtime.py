...
OUT=Path('docs/handoffs/mega-v'); TMP=Path('.tmp/mega-v')
...
def sentinel(port,model,kind):
 row={'model':model,'runtime':kind,'requiredConsecutive':20,'attempts':[]}
 report['attempts'].append(row)
 for i in range(20):
  start=time.monotonic()
  try:
   if kind=='clean-ollama':
    data=request(port,'/api/chat',{'model':model,'stream':False,'think':False,'keep_alive':'2m','options':{'num_gpu':0,'num_ctx':512,'num_predict':20,'temperature':0},'messages':[{'role':'user','content':'Reply with exactly VIGIA_OK. No other text.'}]})
 ...
 row['passed']=len(row['attempts'])==20 and all(x['success'] for x in row['attempts']);save();print(kind,model,'PASS' if row['passed'] else 'FAIL',flush=True)
 return row['passed']
def wait_health(port,path):
 ...
 return False
env={**os.environ,'OLLAMA_HOST':'127.0.0.1:11436','OLLAMA_MODELS':str(base),'OLLAMA_LLM_LIBRARY':'cpu','OLLAMA_CONTEXT_LENGTH':'512','OLLAMA_NUM_PARALLEL':'1','OLLAMA_MAX_LOADED_MODELS':'1','OLLAMA_NO_CLOUD':'true','LLAMA_ARG_FIT':'off','LLAMA_ARG_DEVICE':'none','LLAMA_ARG_OP_OFFLOAD':'0'}
with (TMP/'clean-ollama.log').open('w') as log:
 server=subprocess.Popen(['ollama','serve'],env=env,stdout=log,stderr=log)
 try:
  if wait_health(11436,'/api/version'):sentinel(11436,'qwen3:0.6b','clean-ollama')
  else:report['diagnostics']['cleanServer']='STARTUP_FAILED';save()
 finally:
  server.terminate()
  try:server.wait(timeout=8)
  except subprocess.TimeoutExpired:server.kill();server.wait()
# Direct installed native runner isolates Ollama orchestration and disables both
# tensor and operation offload. No new model or external runtime is downloaded.
runner='/opt/homebrew/Cellar/ollama/0.30.7_1/libexec/lib/ollama/llama-server'
for model in ['qwen3:0.6b','llama3.2:3b']:
  ...
  try:
   if wait_health(11437,'/health'):
    ...
    if passed and model=='llama3.2:3b':
     ...
     subprocess.run(['node','scripts/mega_v_model_corpus.mjs'],env={**os.environ,'VIGIA_EVAL_LOCAL_PORT':'11437','VIGIA_EVAL_LOCAL_MODEL':model},check=False)
