...
report=json.loads((OUT/'ui-qa.json').read_text());report['checks'].append({'firstAttemptFailure':'operator_request_timeout returning to Brief me on mobile','retry':'Fresh admitted browser; rerun mobile interactions'})
with sync_playwright() as p:
 ...
 for width,height in [(390,844)]:
  ...
  if page.locator('[data-action="situation-community"]').count():
   page.get_by_text('Nearby communities',exact=True).click();page.locator('[data-action="situation-community"]').first.click()
   wait_dom('()=>document.querySelector("[data-situation-output]")?.textContent.includes("Support for this community")');shot('community-'+str(width))
   page.locator('[data-action="situation-preset"][data-question="Brief me"]').click();wait_dom('()=>document.querySelector("[data-situation-output]")?.textContent.includes("Retained situation")')
  page.get_by_text('Operational coverage',exact=True).click();page.locator('[data-action="situation-coverage"][data-category="emergency_hospital"]').click()
  wait_dom('()=>document.querySelector(".situation-map-mode")||document.querySelector("[data-situation-output]")?.textContent.includes("No current calculated community routes")')
  if page.locator('.situation-map-mode').count():
   page.locator('main .tile-map').scroll_into_view_if_needed()
   collision=page.evaluate('()=>{const chip=document.querySelector(".situation-map-mode"),r=chip.getBoundingClientRect();return [...document.querySelectorAll("main .tile-map button")].filter(b=>!chip.contains(b)).filter(b=>{const x=b.getBoundingClientRect();return x.width>0&&x.height>0&&x.left<r.right&&x.right>r.left&&x.top<r.bottom&&x.bottom>r.top}).map(b=>b.getAttribute("aria-label")||b.textContent)}')
   report['checks'].append({'viewport':width,'coverageControlsOverlappingLabel':collision})
   shot('healthcare-coverage-'+str(width));page.locator('.situation-map-mode button').click();page.get_by_role('button',name='Ask Vigia',exact=True).click()
  else:report['checks'].append({'viewport':width,'coverage':'NO_CURRENT_RETAINED_PATHS'})
  ...
  page.get_by_role('button',name='Ask Vigia',exact=True).click();page.locator('[data-mode="scenario"]').click();page.locator('.vg-select-trigger[aria-label^="Additional failure:"]').click();page.get_by_role('option',name='Another facility unavailable',exact=True).click();page.get_by_role('button',name='Run scenario',exact=True).click();wait_dom('()=>document.querySelector("[data-situation-output]")?.textContent.includes("Operational records are unchanged")');shot('scenario-'+str(width))
