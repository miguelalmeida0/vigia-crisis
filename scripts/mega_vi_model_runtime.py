...
OUT=Path('docs/handoffs/mega-vi'); TMP=Path('.tmp/mega-vi')
...
def wait_health(port,path):
 ...
 return False
env={**os.environ,'OLLAMA_HOST':'127.0.0.1:11436','OLLAMA_MODELS':str(base),'OLLAMA_CONTEXT_LENGTH':'512','OLLAMA_NUM_PARALLEL':'1','OLLAMA_MAX_LOADED_MODELS':'1','OLLAMA_NO_CLOUD':'true',}
  ...
  try:
   if wait_health(11437,'/health'):
    ...
    if passed and model=='llama3.2:3b':
     ...
     subprocess.run(['node','scripts/mega_vi_model_corpus.mjs'],env={**os.environ,'VIGIA_EVAL_LOCAL_PORT':'11437','VIGIA_EVAL_LOCAL_MODEL':model},check=False,timeout=550)
