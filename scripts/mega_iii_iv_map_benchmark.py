...
ROOT=Path('/workspace');OUT=Path('/output/acceleration');OUT.mkdir(parents=True,exist_ok=True)
...
HOOK="""async()=>{
 window.__bench=window.__bench||{events:[],seen:new WeakSet(),gl:null};const b=window.__bench;
...
with sync_playwright() as p:
 ...
 for cold in range(int(os.environ.get('COLD_SAMPLES','3'))):
  ...
  for label,route in sequence:
   ...
   results.append(result);(OUT/(LANE+'-map.json')).write_text(json.dumps({'browser':browser.version,'cache':'enabled; transparent forwarding hop','viewport':[1728,966],'results':results,'errors':errors},indent=2));print(label,round(useful or -1),instance,len(req),flush=True)
  context.close()
 browser.close()
server.shutdown()
