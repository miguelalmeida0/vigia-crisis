...
captures=Path('/Users/malmeida/.codex/visualizations/2026/09/08/01a08086-9c18-7c51-b96e-12ccfed16259/mega-vii')
out=root/'docs/handoffs/mega-vii/visual';out.mkdir(exist_ok=True)
for p in captures.glob('*.png'):
 if p.name.startswith('final-') or p.name=='fresh-session-discovery.png' or any(p.name.startswith(r+'-') for r in ['command-overview','incidents','incident-detail','fire-activity','response-access','national-awareness']) or p.name=='detail-operational-picture.png':shutil.copy2(p,out/p.name)
for name in ['ui-qa.json','focused-qa.json','routes-qa.json','discovery-qa.json','community-qualified-qa.json','discovery-community-access-attempt.json']:
 p=captures/name
...
rows.append({'route':'national-awareness','runtime':'national-awareness-1672.png','reference':'.impeccable/surfaces/rator-console-src-approved-routes-fire-activity-js.md','state':'TEXT_COMPOSITION_CONTRACT_ONLY','reason':'The retained six-raster pack contains an old Evidence view, not the current National Awareness anatomy. No matching approved national raster was fabricated.'})
