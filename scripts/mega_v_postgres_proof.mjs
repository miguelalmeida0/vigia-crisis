...
try{
  ...
  const retention=await store.retainHistory(at);assert.equal(retention.removed,0);checks.push('retention-does-not-remove-current-snapshots');
  ...
  const factsBefore=(await client.query('SELECT id,payload FROM pg_temp.world_knowledge_fact ORDER BY id')).rows;
  await knowledge.store.enqueue('CONTROLLED_QUEUE','test',{dueAt:at});const claimed=await knowledge.store.claim(at);assert.ok(claimed);await knowledge.store.finish(claimed,{now:at});
  assert.deepEqual((await client.query('SELECT id,payload FROM pg_temp.world_knowledge_fact ORDER BY id')).rows,factsBefore);checks.push('queue-only-transaction-preserves-facility-facts');
  ...
  await writeFile('docs/handoffs/mega-v/postgres-proof.json',JSON.stringify({lane:'CONTROLLED_SESSION_TEMPORARY_TABLES',passed:checks.length,failed:0,checks,extraction:{text:doc.text,candidates:doc.candidates,source:doc.source},operationalFixtureWrites:0},null,2));console.log(JSON.stringify({passed:checks.length,checks}));
