...
export function responseAccess(vm) {
 ...
 return `<div class="field-page response-priority-page" data-vqa="response-access">${FieldBanner(vm,f,{response:true})}${ResponseCoverage(f)}<div class="field-workspace op-layout"><div class="field-primary">${FieldMap(vm,{response:true})}<div class="op-map-key"><strong>Operational Picture</strong><span>Incident · communities · qualified support · calculated routes</span></div></div><aside class="field-rail" aria-label="Operational support">${OperationalSlot()}</aside></div><details class="physical-detail"><summary>Geographic context and other mapped facilities</summary>${NearestSupport(f)}${rail}</details><div class="response-context-row">${AccessConditions(f)}${OfficialNotices(f)}</div>${AccessInterpretation(f)}<div class="response-secondary">${LocationContext(vm,f)}${FieldFeed(f.events,{title:'Recent field & environmental changes',limit:4})}</div></div>`;
}
