...
export function overview(state) {
 ...
 const summaryLabels={activeOfficial:'active official-feed records',thermalHour:'new detections in 1h',activeWarnings:'official weather warnings',highDangerRegions:'high fire-danger municipalities'};
 const summary=(state.physicalSummary??[]).filter(m=>summaryLabels[m.id]&&primaryMetric(m)).map(m=>`${m.display} ${summaryLabels[m.id]}`).join(' · ');
 ...
 ${panel('Priority incidents',priorityRows?`<div class="rows">${priorityRows}</div>`:empty('Priorities unavailable','No incident records were returned.'),{cls:'overview-priorities',ico:'alert',iconTone:'red',sub:'Reported incidents to investigate or monitor.',action:link('View all','navigate','data-route="incidents"')})}
 </div>
