...
const root = process.env.VIGIA_OPERATOR_STATIC_ROOT ? join(packageRoot, process.env.VIGIA_OPERATOR_STATIC_ROOT) : join(packageRoot, 'dist');
if (root !== packageRoot && root !== join(packageRoot, 'dist')) throw new Error('public_operator_static_root_invalid');
...
async function proxyRead(req, res, upstreamPath, { forceGet = false } = {}) {
  ...
  if (!readMethods.has(method)) return writeJson(res, 405, { error:'public_demo_read_only', message:'This public VIGIA deployment is read-only.' }, { allow:'GET, HEAD, OPTIONS' });
...
async function handle(req, res) {
  ...
  if (url.pathname.startsWith('/backend/')) {
    ...
    // The production console normally bootstraps an authenticated operator session with POST.
    // Public-demo mode deliberately converts only this bootstrap request to the existing public GET session endpoint.
