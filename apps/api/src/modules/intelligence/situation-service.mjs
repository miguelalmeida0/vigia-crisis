...
function readableChanges(s,changes){return changes.filter(c=>!(c.kind==='ROUTE_CHANGED'&&c.geometryChanged!==true&&c.restrictionChanged!==true&&['CALCULATED','STALE'].includes(c.previous?.state)&&['CALCULATED','STALE'].includes(c.current?.state)&&c.previous?.state!==c.current?.state&&c.previous?.eta===c.current?.eta)).map(c=>{const route=s.routes.find(r=>r.id===c.subject),facility=s.facilities.find(f=>f.id===(route?.facilityId??c.subject));return {...c,subjectLabel:facility?.canonicalName??s.sources.find(x=>x.id===c.subject)?.name??({nearestHospital:'Nearest mapped hospital',nearestEmergencyHospital:'Nearest verified emergency hospital',nearestCalculatedEmergencyOption:'Lowest calculated hospital route',nearestVerifiedFireStation:'Nearest verified fire-response facility',windSpeed:'Wind speed',temperature:'Temperature',humidity:'Humidity'}[c.subject])??'Incident relationship'};});}
...
export class SituationService{
  ...
  async rebuild(job){
    ...
    this.counters.captured++;this.counters.recalculations++;if(!before||hash(this.gaps(before))!==hash(this.gaps(next)))await this.huntGaps(next);return stored;
  }
