...
export async function createServices({ config, hub, releaseIdentity, fetchImpl = null, clock = () => new Date(), onStartupPhase = null }) {
  ...
  beginStartupPhase('core_storage');const physicalTruthStore=new PostgresPhysicalTruthStore({databaseUrl:config.databaseUrl,clock,connectionTimeoutMs:config.databaseConnectionTimeoutMs,statementTimeoutMs:config.databaseStatementTimeoutMs,lockTimeoutMs:config.databaseLockTimeoutMs}),physicalTruthStatus=await physicalTruthStore.initialize();
  const deploymentIdentityStore=new PostgresReleaseDeploymentIdentityStore({databaseUrl:config.databaseUrl,releaseIdentity,clock,connectionTimeoutMs:config.databaseConnectionTimeoutMs,statementTimeoutMs:config.databaseStatementTimeoutMs});await deploymentIdentityStore.initialize();
  const operationsStore=new PostgresAlertStore({databaseUrl:config.databaseUrl,clock,connectionTimeoutMs:config.databaseConnectionTimeoutMs,statementTimeoutMs:config.databaseStatementTimeoutMs});await operationsStore.initialize();
  ...
  let intelligenceService=null,operationalEventService=null,operationalIntelligenceService=null,agent1CompatibilityReconciler=null;
  const projectCanonicalTwin=async(snapshot)=>{
    if(!agent1CompatibilityReconciler||!Array.isArray(snapshot?.events))return null;
    try{return await agent1CompatibilityReconciler.reconcile(snapshot.events,{receivedAt:clock(),projectedAt:clock(),budgetMs:config.agent1CompatibilityBudgetMs});}
  ...
  beginStartupPhase('canonical_twin_projection');operationalIntelligenceService=new OperationalIntelligenceService({journal:operationalEventJournal,sourceRegistry:canonicalSourceRegistry,adapters:[agent1Adapter,new FieldNetOperationalAdapter(),new FirmsOperationalAdapter(),new FirmsOperationalAdapter({id:'sentinel3-slstr',sourceId:'sentinel3-slstr',upstreamOrigin:'Sentinel-3 SLSTR L2 FRP',authority:'Copernicus Data Space Ecosystem / ESA',producerId:'ESA-COPERNICUS'}),new CapOperationalAdapter()],clock});
  ...
  const incidentCommandRepository=new PostgresIncidentCommandRepository({databaseUrl:config.databaseUrl,clock});
  ...
  const operationalIntelligenceRepository=config.databaseUrl?new PostgresOperationalIntelligenceRepository({databaseUrl:config.databaseUrl,clock}):null;
  ...
  const intelligenceSnapshotRepository=new PostgresIntelligenceRepository({databaseUrl:config.databaseUrl,clock});
