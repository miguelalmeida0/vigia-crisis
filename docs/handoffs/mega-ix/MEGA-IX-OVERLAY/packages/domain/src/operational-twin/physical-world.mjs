import {applicableIpmaWarnings} from './warning-applicability.mjs';
import {operationalMateriality} from './operational-materiality.mjs';
...
import { weatherMetrics, weatherProjection, thermalMetrics, airMetrics, warningMetrics, placeMetrics } from './physical-conditions.mjs';
...
export function incidentPhysicalWorld(item,asOf,all=[],scene=null) {
  ...
  const result={incidentId:item.incident.id,asOf,location,currentness:item.operationalTruth?.currentness??item.currentness,locationLabel:item.incident.operatorIdentity?.label??item.incident.label??item.incident.name??item.incident.location?.label??(location?`Incident near ${location[1].toFixed(4)}, ${location[0].toFixed(4)}`:'Location not reported'),weather,thermal,air:{metrics:air.metrics,smoke:air.smoke},warnings,places,status,
...
export function projectPhysicalWorld({twin,actor,incidentId,world=null,scene=null,asOf=twin?.asOf}) {
  ...
  const projectWeather=weatherProjection(stationRecords(world),asOf),incidents=selected.map(i=>{
    const result=incidentPhysicalWorld(i,asOf,authorized,scene);
    if(world&&result.location)result.weather=projectWeather({location:result.location,subjectId:i.incident.id,sourceState:world.sources?.ipmaWeather?.error?'unavailable':world.sources?.ipmaWeather?.state});
    ...
    const identity=i.incident?.operatorIdentity??i.incident??{},sourceNamed=rows(world?.fires).find(r=>canonicalIncidentId(r.id??'')===canonicalIncidentId(i.incident.id)),district=identity.district??i.incident?.location?.district??sourceNamed?.district;
    if(world){const direct=rows(world.directWarnings),useDirect=direct.length>0||world.sources?.ipmaWarnings?.state==='current',applicable=applicableIpmaWarnings({records:useDirect?direct:world.warnings,location:result.location,district,areaId:identity.warningAreaId,areas:world.warningAreas??[],source:world.sources?.[useDirect?'ipmaWarnings':'warnings']??{},asOf});if(applicable.matchedAreaIds.length)result.warnings=applicable;}
    ...
    for(const w of result.warnings.warnings.filter(w=>w.state==='ACTIVE'&&timestamp(w.effectiveAt??w.effective)>=timestamp(asOf)-3*3600000))result.changes.push({id:w.id,at:w.effectiveAt??w.effective,type:'Warning',source:w.issuer??w.authority,where:w.area,incidentId:result.incidentId,material:true,text:`${w.issuer??w.authority} ${w.type??'warning'} is effective for ${w.area??'this incident area'} until ${w.expiresAt??w.expires}.`,evidence:[w.id]});
    const lifecycle=rows(twin.materialChanges).filter(c=>c.type==='incident_currentness_changed'&&canonicalIncidentId(c.entity)===canonicalIncidentId(result.incidentId)).map(c=>({...c,at:c.timestamp,incidentId:result.incidentId,type:'Incident update',text:c.summary,material:true,source:'Incident observation history'}));
    result.changes=operationalMateriality([...result.changes,...lifecycle],{asOf}).events;
  ...
  if(national){
    const current=authorized.filter(i=>(i.operationalTruth?.currentness??i.currentness)?.countsAsCurrent);
    summaryMap.set('activeOfficial',metric('activeOfficial','Current incident records',current.length,'',{source:'Attributed fire observations',measurementType:'DERIVED',basisAt:asOf,definition:{statistic:'Records inside the configurable fire-activity queue window; verification is separate'}},asOf,null,'Current candidates and verified incidents; stored historical records are excluded.'));
  ...
  const result={schemaVersion:'vigia.physical-world.v2',asOf,national,incidents:incidentId?incidents:incidents.map(i=>({incidentId:i.incidentId,asOf,metrics:listMetrics(i),station:i.weather.station,changes:i.changes.slice(0,6)})),summary:[...summaryMap.values()],currentnessCounts:twin?.operationalTruth?.currentnessCounts??null,activity,
