...
// Explicit operator selection only: never substitute the device's location or
// silently infer a report point from an incident/road name.
export function pickFieldReportPoint(host) {
  const map=host?.matches?.('.tile-map')?host:host?.querySelector?.('.tile-map'),instance=mapInstances.get(map);
  if(!instance)return Promise.reject(Error('The map is not ready. Enter the observation location manually.'));
  return new Promise((resolve,reject)=>{
    const gl=instance.gl,canvas=gl.getCanvas(),previous=canvas.style.cursor,notice=document.createElement('div');notice.className='situation-map-mode';notice.textContent='Select the observed location · Enter uses map centre · Escape cancels';map.append(notice);canvas.style.cursor='crosshair';map.focus();
    const clean=()=>{clearTimeout(timer);canvas.removeEventListener('click',click,true);map.removeEventListener('keydown',key,true);canvas.style.cursor=previous;notice.remove();};
    const finish=point=>{clean();resolve([Number(point.lng.toFixed(6)),Number(point.lat.toFixed(6))]);};
    const click=event=>{event.preventDefault();event.stopImmediatePropagation();const rect=canvas.getBoundingClientRect();finish(gl.unproject([event.clientX-rect.left,event.clientY-rect.top]));};
    const key=event=>{if(event.key==='Enter'){event.preventDefault();event.stopImmediatePropagation();finish(gl.getCenter());}else if(event.key==='Escape'){event.preventDefault();event.stopImmediatePropagation();clean();reject(Error('Location selection cancelled.'));}};
    const timer=setTimeout(()=>{clean();reject(Error('Location selection expired. Select a point again.'));},90000);
    canvas.addEventListener('click',click,true);map.addEventListener('keydown',key,true);
  });
}
