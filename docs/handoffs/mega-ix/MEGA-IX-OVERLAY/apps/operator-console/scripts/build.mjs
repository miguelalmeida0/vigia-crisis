...
await Promise.all([
  ...
  cp(path.join(root, 'field-team-sw.js'), path.join(dist, 'field-team-sw.js')),
  cp(path.join(root, 'offline-team.html'), path.join(dist, 'offline-team.html')),
]);
...
const offlineEntry=Object.entries(bundled.metafile.outputs).find(([,value])=>value.entryPoint?.endsWith('src/offline-team.js'))?.[0];
if(!offlineEntry)throw new Error('offline_bundle_entry_missing');
await atomicWriteNoFollow(path.join(dist,'offline-team.html'),(await readFile(path.join(root,'offline-team.html'),'utf8')).replace('./src/offline-team.js','./'+path.relative(dist,path.resolve(offlineEntry)).replaceAll(path.sep,'/')),{root:projectRoot});
const assetEntries=await hashedEntries((await files(dist)).filter((file)=>path.basename(file)!=='build-manifest.json'),dist);
await atomicWriteNoFollow(path.join(dist,'offline-assets.json'),JSON.stringify({assets:(await files(dist)).map(f=>'/'+path.relative(dist,f).replaceAll(path.sep,'/')).filter(f=>['/index.html','/offline-team.html'].includes(f)||/^\/(bundle|styles|assets)\//.test(f))}),{root:projectRoot});
assetEntries.push(...await hashedEntries([path.join(dist,'offline-assets.json')],dist));
