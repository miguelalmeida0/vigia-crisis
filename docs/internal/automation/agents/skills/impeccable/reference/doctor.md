Report and repair drift between this project's Impeccable artifacts and what the installed version reads: docs/product/PRODUCT.md, docs/product/DESIGN.md and its `docs/internal/automation/impeccable/design.json` sidecar, `docs/internal/automation/impeccable/config.json`, persisted surface briefs, and the design hook.
...
```
docs/internal/automation/agents/skills/impeccable/scripts/impeccable doctor --json
```
...
- **`auto`** carries no decision. Run `docs/internal/automation/agents/skills/impeccable/scripts/impeccable doctor --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
